...
uint32_t bit_rates=MOTOR_DSHOT600_SYMBOL_RATE*3; //dshot把一个bit分成了3份
#endif
#ifdef BB_UART
uint32_t bit_rates=BAUDRATE*SAMPLERATE;
#endif
/* TIM1 init function */
void MX_TIM1_Init(void)
{
  ...
  htim1.Instance = TIM1;
  htim1.Init.Prescaler = SystemCoreClock/bit_rates/20-1;  //SystemCoreClock/MOTOR_DSHOT600_SYMBOL_RATE：600K的分频。/10：每一部分又再细分成10份，-1：实际值-1放到寄存器中
  htim1.Init.CounterMode = TIM_COUNTERMODE_UP;
  htim1.Init.Period = 19;   //实际周期n+1
...
uint8_t rx[256]={0},rcnt=0;
void tim1_ch2_dma_callback(DMA_HandleTypeDef *hdma)
{
    __HAL_GPIO_EXTI_CLEAR_FLAG(GPIO_PIN_6);                     //先清除中断标志，在接收数据期间实际上触发了多次置位了标志。
    EXTI->IMR|=GPIO_PIN_6;
    //HAL_NVIC_EnableIRQ(EXTI9_5_IRQn);
    __HAL_TIM_DISABLE_DMA(&htim1, TIM_DMA_CC2);
    rx[rcnt++]=bb_uart_rx_decode(bb_uart_rxbuff,GPIO_PIN_6);
}
/* USER CODE END 1 */
